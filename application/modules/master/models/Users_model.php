...
class Users_model extends MY_Model
{
    ...
    {
        // numpang exsekusi sql mode 
        $sql = "SET GLOBAL sql_mode=(SELECT REPLACE(@@sql_mode,'ONLY_FULL_GROUP_BY','')); ";
        $this->db->query($sql);
        
        $sql =' select * from m_user where username="'.$usr.'" and password="'.$pwd.'" ';
    ...
    function detail_user($usr,$pwd){
    	$query = $this->db->query(' select * from m_user where username="'.$usr.'" and password="'.$pwd.'" ')->row();
