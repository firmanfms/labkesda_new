          ...
          <div class="box-body">
            <div class="form-group">
              ...
              <div class="col-sm-5">
                ...
                <input type="hidden" id="id_keterangan" value='<?= $detail->id_keterangan; ?>'>
              </div>
            </div>
            <div class="form-group">
              ...
              <div class="col-sm-5">
                <select class="form-control select2" style="width: 100%;" id='versi'>
                  <option disabled="" selected="">- Pilih -</option>
                  <option value="1" <?= ($detail->versi=='1')? 'selected' : ''; ?>>- Satu -</option>
                  <option value="2" <?= ($detail->versi=='2')? 'selected' : ''; ?>>- Dua -</option>
                </select>
              </div>
            </div>
            <div class="form-group">
              ...
              <div class="col-sm-9">
                <textarea name="content" id="ckeditor1" required><?= $detail->keterangan; ?></textarea>
              </div>
            </div>
            <div class="form-group">
              <label for="inputEmail3" class="col-sm-2 control-label">Catatan </label>
              <div class="col-sm-9">
                <textarea name="content" id="ckeditor2" required><?= $detail->catatan; ?></textarea>
              </div>
            </div>
              <div class="form-group">
              <label for="inputEmail3" class="col-sm-2 control-label">Keterangan 2</label>
              <div class="col-sm-7">
                <textarea name="content" rows="7" cols="100" id="keterangan2" required><?= $detail->keterangan2; ?></textarea>
              </div>
            </div>
            <div class="form-group">
              <label for="inputEmail3" class="col-sm-2 control-label">Catatan 2</label>
              <div class="col-sm-5">
                <textarea name="content" rows="7" cols="100" id="catatan2" required><?= $detail->catatan2; ?></textarea>
              </div>
            </div>
          </div>
          </div>
...
$(document).ready(function(){
  ...
  function(e){
    ...
    var text1 = html.replace(/<[^>]+>/g, '').replace(/&nbsp;/g, ' '); // buang HTML tags dan replace &nbsp;
    console.log(text2); // output: plain text 
     var html = CKEDITOR.instances['ckeditor2'].getData();
    var text2 = html.replace(/<[^>]+>/g, '').replace(/&nbsp;/g, ' '); // buang HTML tags dan replace &nbsp;
    // var text2 = html.replace(/<[^>]+>/g, ''); // buang HTML tags
    console.log(text2); // output: plain text 
    $.ajax({
      url: baseUrl+'master/keterangan/edit_act',
      type : "POST",  
      data: {
        id_keterangan   : $('#id_keterangan').val(),
        kd_lab          : $('#kd_lab').val(),
        versi           : $('#versi').val(),
        keterangan      : CKEDITOR.instances['ckeditor1'].getData(),
        catatan         : CKEDITOR.instances['ckeditor2'].getData(),
        //  keterangan2      : text1 , // CKEDITOR.instances['ckeditor1'].getClipboardData().html,
        // catatan2         : text2, // CKEDITOR.instances['ckeditor2'].getClipboardData().html,
