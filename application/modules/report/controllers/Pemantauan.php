...
class Pemantauan extends MY_Controller {
    ...
    function index(){  
        $this->session->set_userdata('ses_menu', array('active_menu' => 'Report', 'active_submenu' => 'pemantauan', 'active_submenu2' => ''));

        $query                  = "SELECT a.kd_parameter,a.kd_sampel,b.nm_parameter,b.satuan FROM t_mutu_internal a 
                                    LEFT JOIN m_parameter b ON a.kd_parameter=b.kd_parameter
                                    WHERE a.aktif='Y' GROUP BY a.kd_parameter";
        $data['data_parameter'] = $this->db->query($query)->result();
        ...
        if($this->input->post('kd_parameter')!=''){
            ...
            if($kd_lab=='LK'){
                ...
                $lab        = 'Lab Makanan dan Minuman';
            }

            $query_pencarian    = "SELECT a.*,b.nm_parameter,b.satuan,c.nm_sampel,a.metode_analisa FROM t_mutu_internal a 
                                    LEFT JOIN m_parameter b ON a.kd_parameter=b.kd_parameter
                                    LEFT JOIN m_sampel c ON a.kd_sampel=c.kd_sampel
                                    WHERE a.kd_parameter='".$kd_parameter."' ";
            $hasil              = $this->db->query($query_pencarian)->result();
            $nm_parameter       = $hasil[0]->nm_parameter;
			$no_pendaftaran     = $hasil[1]->no_pendaftaran;


        }

        $data['no_pendaftaran']	= $no_pendaftaran;
		$data['kd_parameter']   = $kd_parameter;
        $data['nm_parameter']   = $nm_parameter;
        $data['data_mutasi']    = $hasil;
        $data['lab']            = $lab;
        $data['data_lab']       = $this->laboratorium_model->get_all();

        $this->template->load('body', 'report/pemantauan',$data);
    }

    function get_parameter(){
        $lab            = $this->input->post('id');
        $query                  = "SELECT a.kd_parameter,a.kd_sampel,b.nm_parameter,b.satuan,b.kd_lab FROM t_mutu_internal a 
                                    LEFT JOIN m_parameter b ON a.kd_parameter=b.kd_parameter
                                    WHERE a.aktif='Y' AND kd_lab='".$lab."' GROUP BY a.kd_parameter";
        $data_parameter         = $this->db->query($query)->result();

        $result                 = $data_parameter;
        echo json_encode($result);
    }

    function cetak($kd_parameter,$cetak){

        $query_pencarian    = "SELECT a.*,b.nm_parameter,b.satuan,c.nm_sampel FROM t_mutu_internal a 
                                LEFT JOIN m_parameter b ON a.kd_parameter=b.kd_parameter
                                LEFT JOIN m_sampel c ON a.kd_sampel=c.kd_sampel
                                WHERE a.kd_parameter='".$kd_parameter."' ";
        $hasil              = $this->db->query($query_pencarian)->result();
        ...
        $data['kd_parameter']   = $kd_parameter;
        $data['nm_parameter']   = $hasil[0]->nm_parameter;
        $data['data_mutasi']    = $hasil;
        $data['cetak']          = $cetak;

        $this->load->view('report/pemantauan_cetak',$data);
    }
