...
class Lab_lingkungan extends MY_Controller {
    ...
    function print(){  
        ...
        if($this->input->post('tgl_dari')!=''){
            ...
            // TODOS: Sesuaikan lagi ke query atas. Query ini untuk Testing saja.
            // $data['pendaftaran']    = $this->db->query("
            // SELECT a.nama,a.alamat,a.`no_pendaftaran`,a.`tgl_diterima`,b.nm_sampel,a.kd_lab
            //     FROM t_pendaftaran a 
            // LEFT JOIN m_sampel b ON a.`kd_sampel`=b.`kd_sampel`
            ...
            // NOTE : Logic untuk Matriks Parameter dan Nilai 
            $listNoPendaftaran = [];
            foreach ($data['pendaftaran'] as $key => $value) {
                $listNoPendaftaran[] = $value->no_pendaftaran;
            }
            $detailPendaftaran = [];
            $resultPendaftaranDetail = $this->db->query("
                SELECT a.no_pendaftaran, a.kd_parameter, a.nilai
                    FROM t_pendaftaran_detail a 
                WHERE a.no_pendaftaran IN ('".implode("','",$listNoPendaftaran)."');
            ")->result();
            foreach ($resultPendaftaranDetail as $key => $value) {
                $detailPendaftaran[$value->no_pendaftaran][$value->kd_parameter] = $value;
            }
            foreach ($data['pendaftaran'] as $key => $value) {
            ...
            $titleParam = $this->db->query("
                SELECT DISTINCT(a.kd_parameter), mparam.nm_parameter FROM t_pendaftaran_detail a
                    INNER JOIN m_parameter mparam ON mparam.kd_parameter = a.kd_parameter
                WHERE a.no_pendaftaran IN ('".implode("','",$listNoPendaftaran)."');
            ")->result();
            $data['title_param'] = $titleParam;
            // END
