        ...
        <div class="box-body">
          <table id="itemsTable" class="table table-bordered table-striped">
            ...
            <tr>
              ...
              <td><?= $status; ?></td>
              <td align="center">
                <a href="<?php echo base_url('transaksi/hasil_lingkungan/detail/'.$value->nopendaftar.'/'.$value->max_urutan); ?>" type="submit" class="btn btn-xs btn-success">Detail</a>
                <?php 
                if($value->status_approve_query==3){
                ?>
                  <a href="<?php echo base_url('transaksi/hasil_lingkungan/insert/'.$value->nopendaftar.'/'.$value->max_urutan); ?>" type="submit" class="btn btn-xs btn-warning">Hasil</a>
                  <?php
                }else if($value->status_approve_query==1 OR $value->status_approve_query==4){
                ?>
                  <a href="<?php echo base_url('transaksi/hasil_lingkungan/update/'.$value->nopendaftar.'/'.$value->max_urutan); ?>" type="submit" class="btn btn-xs btn-warning">Hasil</a>
                <?php
                }
                ?>
                <?php
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLabel">Print Lingkungan</h5>
      </div>
      <form action="<?= base_url("transaksi/hasil_lingkungan/print") ?>" method="post" target="_blank">
        <div class="modal-body">
          ...
          <div class="row">
            ...
            <label class="radio-inline col-sm-3">
              <input type="radio" name="ttl" id="ttl" value="koor"> Koordinator Lingkungan
            </label>
          </div>
        </div>
...
function refreshgrid(){
  ...
  var url = "<?php echo base_url('transaksi/hasil_lingkungan/'); ?>";
  url += "?tahun=" + tahun;
            ...
            $.ajax({
                ...
                success : function(resp){
                  if(resp.status == 'ERROR INSERT' || resp.status == false) {
                    ...
                    setTimeout(function () {
                      window.location.href = baseUrl+'transaksi/hasil_lingkungan'; //will redirect to google.
                    }, 2000);
                  }
                }
            });
